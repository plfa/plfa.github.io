  ...
  for line in cb.text:gmatch('[^\n]+') do
    if line:sub(1,2) ~= '//' then
      ...
      if not fh then
        ...
        -- if there is a title in the frontmatter, add a level-1 heading
        if doc.meta.title ~= nil then
            blocks:insert(pandoc.Header(1, doc.meta.title))
        end
        -- add the contents
        local contents = doc.blocks
        ...
        -- update header level and relative urls
        contents = update_contents(contents, shift_heading_level_by, path.directory(line))
