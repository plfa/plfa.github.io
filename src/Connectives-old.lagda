...
Given two propositions `A` and `B`, their *conjunction* holds
if both `A` holds and `B` holds.  In logic, conjunction is often
written `A & B` or `A ∧ B`, but here we write it `A × B`, and
refer to it as the *product* of the two corresponding types.
We formalise this notion with a suitable inductive type.
\begin{code}
data _×_ : Set → Set → Set where
  _,_ : ∀ {A B : Set} → A → B → A × B
\end{code}
Evidence that `A × B` holds is of the form `(M , N)`, where `M` is
evidence that `A` holds and `N` is evidence that `B` holds.  Such a
term is called a *pair*. By convention, we always parenthesise
pairs, even though `M , N` is also accepted by Agda.

Given that the conjunction of `A` and `B` holds,
we can conclude that `A` holds,
and we can also conclude that `B` holds.
\begin{code}
proj₁ : ∀ {A B : Set} → A × B → A
proj₂ (x , y) = x
...
proj₂ (x , y) = y
\end{code}
If `L` is evidence that `A × B` holds, then `proj₁ L` is evidence
...
We set the precedence of product so that it binds less tightly than
anything save sum (to be introduced below), and of pairing so that it
binds less tightly than any arithmetic operator.
\begin{code}
infixr 2 _×_
infixr 4 _,_
\end{code}
...
In set theory, product is also sometimes
called the *cartesian product*, and in computing it corresponds
to a *record* type.  Products over types behave similarly to
products of numbers.  In particular, the size of the product of
two finite types is the product of the size of each type.
...
data Tri : Set where
  ...
  cc : Tri
\end{code}
Then the type `Bool × Tri` has six members, the product of two and three:
...
to permit simplification.  For instance, replacing the definition of
`from∘to` by `λ{ w → refl }` will not work.
\begin{code}
...
the two types.  For instance, `(true , aa)`, a member of the
former, corresponds to `(aa , true)`, a member of the latter.
...
instance `((1 , true) , aa)`, a member of the former,
corresponds to `(1 , (true , aa))`, a member of the latter.


## Truth is unit

Truth always holds. In logic, truth is often written `⊤`.
We use the same notation here, and refer to is as the *unit* type.
We formalise this notion with a suitable inductive type.
\begin{code}
data ⊤ : Set where
  tt : ⊤
...
Whereas given `A × B` one can conclude from it either `A` or `B`,
there is nothing more one can conclude from `⊤`.
Given that `⊤` holds, there is nothing more of interest we
