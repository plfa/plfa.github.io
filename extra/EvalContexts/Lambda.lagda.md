...
indicate that it is a value. In Agda, we must
invoke the `Value` predicate, but we use `{{ }}`
so that it may be invoked implicitly.
...
data Frame : Set where
  ...
  `suc□                 : Frame
  case□[zero⇒_|suc_⇒_]  : Term → Id → Term → Frame

_[_] : Frame → Term → Term
(□· M) [ L ]                        =  L · M
(V ·□) [ M ]                        =  V · M
(`suc□) [ M ]                       =  `suc M
(case□[zero⇒ M |suc x ⇒ N ]) [ L ]  =  case L [zero⇒ M |suc x ⇒ N ]
```
